...
| avifenc / avifdec (`libavif-tools`) | AVIF transparent (détourage + AVIF) + contrôle de l'alpha |
...
- **Détourage + AVIF** — rembg puis AVIF en conservant la transparence via `avifenc` (jamais ffmpeg, qui perd l'alpha) ; presets de qualité Haute/Équilibrée/Léger, alpha vérifié après encodage
